  ...
  <div class="w-full">
    ...
    <v-stepper v-model="step" vertical>
        ...
        <v-card flat>
          <v-card-text>
            <div class="en-merchantForms-content-firstSection">
              <merchantForm ref="mForm" :validateMerchant="validateMerchant" @merchantValidated="merchantFormValidated"
                            :show-map="showMap"/>
            </div>
          </v-card-text>
        </v-card>
      ...
      <v-stepper-content step="2">
        <v-card flat>
          <v-card-text>
            <contactPerson ref="pForm" :validateContactPerson="validatePerson"
                           @contactPersonValidated="contactPersonFormValidated" :show-map="showMap"/>
          </v-card-text>
        </v-card>
        <v-btn
          v-if="!branches.exist"
          ...
          color="#dc1f29"
          dark
          @click="validatePersonForm">Submit The Form without Branches
        </v-btn>
    ...
    methods: {
      ...
      merchantFormValidated(status) {
        ...
        if (!data.location.lat || !data.location.long) {
          this.$refs.snackbar.open("Sorry, Please fill the location field", 4500, 'error');
          return;
        }
      ...
      sendDataWithoutBranches() {
        ...
        if (
          ...
          const asyncSubmitting = async () => {
            await this.$axios.post("/merchants", merchantData, config).then((result) => {
              this.$axios.post("/payments", {merchant : result.data.data._id, payment : this.$store.state.paymentMerchData}, config).then((res) => {
                console.log(res)
                this.success = "Added Successfully!"
                this.loading = false
                this.$refs.snackbar.open("Added Successfully!", 4500, 'info');
